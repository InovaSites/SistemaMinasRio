...
document.addEventListener("DOMContentLoaded", () => {
    ...
    const dataAtualInput = document.querySelector(".data_atual");
    const quantDiasInput = document.querySelector(".quant_dias");
    const vencimentoInput = document.querySelector("input[name='vencimento']");

    // Bloqueia o campo de vencimento para edição manual
    vencimentoInput.setAttribute("readonly", true);

    function calcularVencimento() {
        const dataAtual = new Date(dataAtualInput.value); // Pega a data inicial
        const quantDias = parseInt(quantDiasInput.value) || 0; // Pega a quantidade de dias

        if (!isNaN(dataAtual.getTime())) { // Verifica se a data é válida
            const novaData = new Date(dataAtual); // Cria uma cópia da data inicial
            novaData.setDate(novaData.getDate() + quantDias); // Adiciona os dias

            // Formata a nova data para YYYY-MM-DD
            const dataFormatada = novaData.toISOString().split("T")[0];
            vencimentoInput.value = dataFormatada; // Atualiza o input de vencimento
        }
    }

    // Adiciona eventos para recalcular o vencimento sempre que os valores mudarem
    dataAtualInput.addEventListener("change", calcularVencimento);
    quantDiasInput.addEventListener("input", calcularVencimento);
});
...
function calcularValorDA() {
    const totalCargaElement = document.querySelector("#total_carga");
    const totalLiquidoElement = document.querySelector("#total_liquido");
    const adicionalInput = document.querySelector("#adicional_valor");
    const descontoInput = document.querySelector("#desconto_valor");

    // Obtém e formata os valores
    const totalCarga = parseFloat(totalCargaElement.textContent.replace(",", ".")) || 0;
    const adicional = parseFloat(adicionalInput.value.replace(",", ".").replace(/[^0-9.]/g, "")) || 0;
    const desconto = parseFloat(descontoInput.value.replace(",", ".").replace(/[^0-9.]/g, "")) || 0;

    let totalCalculo = totalCarga + adicional - desconto;

    // Atualiza o valor formatado no campo de saída
    totalLiquidoElement.textContent = totalCalculo.toFixed(2).replace(".", ",");
}
