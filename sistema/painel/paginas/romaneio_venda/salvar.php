...
$adicional = $_POST['adicional'] ?? '0.0';
$descricao_a = $_POST['descricao_a'] ?? ''; 

$desconto = $_POST['desconto'] ?? '0.0';
$descricao_d = $_POST['descricao_d'] ?? ''; 
...
if ($id == "") {
	$query = $pdo->prepare("INSERT INTO $tabela SET descricao_a = :descricao_a, adicional = :adicional, descricao_d = :descricao_d, desconto = :desconto, atacadista = :atacadista, quant_dias = :quant_dias, data = :data, nota_fiscal = :nota_fiscal, plano_pgto = :plano_pgto, vencimento = :vencimento, total_liquido = :total_liquido");
} else {
	$query = $pdo->prepare("UPDATE $tabela SET descricao_a = :descricao_a, adicional = :adicional, descricao_d = :descricao_d, desconto = :desconto, atacadista = :atacadista, quant_dias = :quant_dias, data = :data, nota_fiscal = :nota_fiscal, plano_pgto = :plano_pgto, vencimento = :vencimento, total_liquido = :total_liquido where id = '$id'");
}

$adicional = str_replace(',', '.', $adicional);
$desconto = str_replace(',', '.', $desconto);

if ($adicional === '' || !is_numeric($adicional)) {
	$adicional = '0.0'; // Valor padrão para campos decimais
}

if ($desconto === '' || !is_numeric($desconto)) {
	$desconto = '0.0'; // Valor padrão para campos decimais
}
...
$query = $pdo->prepare("INSERT INTO receber SET id_romaneio = :id_romaneio, descricao = :descricao, cliente = :cliente, valor = :valor, vencimento = '$vencimento' $pgto, data_lanc = curDate(), forma_pgto = null, frequencia = 0, obs = :obs, arquivo = '', subtotal = :valor, usuario_lanc = '$id_usuario' $usu_pgto, pago = '$pago', referencia = 'Conta', hora = curTime() ");
